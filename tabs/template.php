...
<div <?= $this->component_attrs() ?>
    ...
    <div role="tablist"
        ...
        <?php foreach ($this->tabs as $index => $tab): ?>
            <?php
            $tab_icon   = $this->render_tab_icon($tab);
            $icon_left  = $this->icon_position !== 'right' ? $tab_icon : '';
            $icon_right = $this->icon_position === 'right' ? $tab_icon : '';
            ?>
            <button role="tab"
                ...
                <?= $icon_left ?><?= esc_html($tab['label'] ?? '') ?><?= $icon_right ?>
            </button>
        <?php endforeach; ?>
